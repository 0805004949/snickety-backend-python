...
## Default JSON encoder는 set을 JSON으로 변환안됨
## 커스텀 인코더 따로 작성해 set->list->json 변환과정생성
...
def login_required(f):
    ...
    def decorated_function(*args, **kwargs):
        access_token = request.hearders.get('Authorization')
        if access_token is not None:
            try:
                payload = jwt.decode(access_token, current_app.config['JWT_SECRET_KEY'], 'HS256')
            except jwt.InvalidAccessErr:
                payload = None
...
def create_endpoints(app, services):
    ...
    tweet_service = services.user_service
